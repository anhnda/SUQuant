...
import os
from .calibration_utils import load_calibration_data, CALIBRATION_DATASETS
...
def get_tokens(dataset_name, split, tokenizer, seq_len, num_samples, save_path=None, seed=None):
    ...
    # ---- calibration_utils path (c4 / redpajama / wikitext2) ----
    if dataset_name in ('c4', 'redpajama', 'wikitext2'):
        logging.info(f"Using calibration_utils loader for {dataset_name}")
        samples = load_calibration_data(
            ...
            cache_dir=os.path.join(os.path.dirname(save_path or "./cache/tokens"), "calibration_cache"),
            return_tensors=True,
        )
        # calibration_utils returns [1, seq_len]; GuidedQuant wants [seq_len]
        tokens = [t.squeeze(0)[:seq_len].cpu() for t in samples]
        assert all(t.shape[0] == seq_len for t in tokens), "bad seq_len from calibration_utils"
    else:
